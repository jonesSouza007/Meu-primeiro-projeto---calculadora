(function(doc, win) {
    ...
    - faça refectories para melhorar esse código, mas de forma que o mantenha com a
    ...
    */
    ...
    const RegexForMultiplication = /\d+\*\d+/g //Operador de multiplicação
    ...
    const RegexForNumberBeforePercentage = /\d+([.]\d+)?\%/g //Percentagem no final
    const RegexForNumberAfterPercentage = /\d+\%\d+/ //Percentagem no meio
    ...
    function PriorityDivOrMult() {
        if (RegexForOtherOperators.test($Visor.value)) {
            OrganizerCalcMultiply(RegexForOtherOperators, RegexForMultiplication)

            OrganizeCalcDivision(RegexForDivision, RegexForValueWithOperators)
        }
    }
    ...
    function CalcDivAndMult() {
        if (RegexForValueWithOperators.test(ValueWithParenteseForCalc)) {
            if (RegexForMultiplication.test(ValueWithParenteseForCalc)) {
                let ValueProvisory = ValueWithParenteseForCalc
                let ValueForMult = ValueWithParenteseForCalc.match(RegexForMultiplication)
                let result
    ...
    function Percentage() {
        if (RegexForPercentageInNumber.test($Visor.value)) {
            ...
            if (RegexForNumberBeforePercentage.test($Visor.value)) {
                let FirstValueWitchPercentage = $Visor.value.match(/\d+(?:[.]\d+)?\%/).join('')
                let firstValuePercentage = FirstValueWitchPercentage.slice(0, -1)
                result = Number(firstValuePercentage) / 100
                ValueFinale = $Visor.value.replace(FirstValueWitchPercentage, result)

            }
            if (RegexForNumberAfterPercentage.test($Visor.value)) {
    ...
    function OrganizerCalcMultiply() {
        if (RegexForMultiplication.test($Visor.value)) {
            let ValueProvisory = $Visor.value
            let ValueForMult = $Visor.value.match(RegexForMultiplication)
            for (let index = 0; index < ValueForMult.length; index++) {
                let ValueForMultCalc = ValueForMult[index].match(RegexForValueWithOperators)
